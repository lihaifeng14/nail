...
public class NewsFragmentAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position == 0 && mContent.hasFocusData()) {
            if (convertView == null) {
                convertView = LayoutInflater.from(mContext).inflate(R.layout.item_pictures, null);
            ...
            ImageLoader.getInstance(mContext).displayImage(
                    data.getThumbnail(), holder.right_image, R.drawable.item_background_default, null);
        }
        return convertView;
    }
    ...
    public class NewsPictureGalleryAdapter extends BaseAdapter 
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            if (convertView == null) {
                convertView = LayoutInflater.from(mContext).inflate(R.layout.item_picture_gallery, null);
            }
            ImageView img = (ImageView)convertView.findViewById(R.id.picture_gallery);
            if (mContent != null && mContent.mFocusData != null) {
                NewsItemData data = mContent.mFocusData.getBody().getItem().get(position);
                ImageLoader.getInstance(mContext).displayImage(data.getThumbnail(), img, R.drawable.item_picture_background_default, null);
            }
            return convertView;
        }
